...
if uploaded_file is not None:
    ...
    # Menu Analisis Data
        ...
        for feature in selected_features:
            fig = px.histogram(dff, x=feature, color="satisfaction", barmode="overlay",
                            title=f"Distribusi {feature} Berdasarkan Satisfaction")
            st.plotly_chart(fig)

        # Pastikan kolom yang dipilih ada di dataset
        if all(feature in dff.columns for feature in selected_features) and 'satisfaction' in dff.columns:
            # Filter data hanya dengan fitur yang dipilih dan target
            dff_filtered = dff[selected_features + ['satisfaction']]
